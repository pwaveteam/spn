...
const handleSelect=(row:any)=>setSelectedIndustry((prev:any)=>(prev?.id===row.id?null:row))
...
<td key={i}className="px-3 py-0 text-center align-middle truncate"style={{minWidth:i<4?150:100,maxWidth:i<4?150:100,borderRight:i===6?'none':'1px solid #CCCCCC',borderBottom:'1px solid #CCCCCC',fontWeight:'normal',color:'#333639',whiteSpace:'nowrap',textOverflow:'ellipsis',overflow:'hidden'}}title={String(row[key as keyof typeof row])}>{row[key as keyof typeof row]}</td>
))}
</tr>
))}
</tbody>
