...
{key:"afterPhoto",label:"평가현장 사진",minWidth:100,renderCell:(_r,rowIdx)=><><input type="file"accept="image/*"style={{display:"none"}}ref={el=>{afterFileRefs.current[rowIdx]=el}}onChange={e=>handleFileChange(e,rowIdx,"after")}/><button type="button"onClick={()=>handleFileClick(rowIdx,"after")}><Upload size={19}/></button></>},
...
<DataTable<RiskDataRow>columns={columns}data={data}onCheckedChange={setChecked}selectable/>
</div>
</div>
<div className="mt-5 flex justify-between">
<Button variant="secondary"onClick={()=>navigate("/risk-assessment/methods/threestep/step1")}><ChevronLeft size={18}className="mr-2"/>이전으로</Button>
<Button variant="secondary"onClick={()=>navigate("/risk-assessment/methods/threestep/step3")}>저장 후 다음으로<ChevronRight size={18}className="ml-2"/></Button>
</div>
</section>
)
}
