...
{id:1,task:"고소작업대 점검",hazard:"작업 중 추락 위험",law:"산업안전보건법 제34조(추락방지 조치)",action:"- 안전대 착용\n- 작업 전 장비 점검",proof:null},
{id:2,task:"사다리 이용",hazard:"사다리 미끄러짐으로 인한 전도",law:"산업안전보건기준에 관한 규칙 제38조",action:"- 미끄럼 방지 고무 부착\n- 수평 유지 확인",proof:null},
]
...
...row,
task:(<EditableTextArea value={row.task}onChange={val=>handleCellChange(row.id,"task",val)}placeholder="작업내용 입력"/>),
...
action:(<EditableTextArea value={row.action}onChange={val=>handleCellChange(row.id,"action",val)}placeholder="조치내용 입력"/>),
...
const addRow=()=>{
setData(prev=>[...prev,{id:prev.length?Math.max(...prev.map(r=>r.id))+1:1,task:"",hazard:"",law:"",action:"",proof:null}])
}
...
<StepBar currentStep={currentStep}setCurrentStep={()=>{}}/>
...
<Button variant="action"onClick={()=>alert("체크리스트 항목 선택")}className="flex items-center gap-1"><FolderSearch size={16}/>항목 선택</Button>
<Button variant="action"onClick={()=>alert("기준자료 추가")}className="flex items-center gap-1"><CirclePlus size={16}/>기준자료 추가</Button>
<Button variant="action"onClick={()=>alert("임시저장 완료")}className="flex items-center gap-1"><Save size={16}/>임시저장하기</Button>
<Button variant="action"onClick={handleDelete}className="flex items-center gap-1"><Trash2 size={16}/>삭제</Button>
</div>
</div>
<DataTable columns={columns}data={dataWithEditableCells}onCheckedChange={setCheckedRows}selectable/>
...
<Button variant="secondary"icon={<ChevronRight size={18}/>}onClick={()=>navigate("/risk-assessment/methods/checklist/step2")}>저장 후 다음으로</Button>
</div>
</section>
)
}
