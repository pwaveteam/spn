...
{key:"result",label:"처리결과",type:"stateToggle",stateOptions:{left:{text:"채택",color:"sky"},right:{text:"미채택",color:"red"}}},
...
const{currentPage,totalPages,currentData,onPageChange}=usePagination<NearMissRow>(data,30)
const{handleDelete,handleDownload,handleSave}=useTableActions<NearMissRow>({data,checkedIds,onDeleteSuccess:ids=>setData(prev=>prev.filter(row=>!ids.includes(row.id))),onSave:()=>{console.log("아차사고 저장",data)}})
