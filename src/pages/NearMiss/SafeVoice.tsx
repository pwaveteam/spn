...
import{CirclePlus,Trash2,Save,QrCode}from"lucide-react"
...
{key:"status",label:"조치여부",type:"stateToggle",stateOptions:{left:{text:"조치",color:"green"},right:{text:"미조치",color:"orange"}}},
...
const{handleCreate,handleDelete,handleDownload,handleSave}=useTableActions({data,checkedIds,onCreate:()=>setModalOpen(true),onDeleteSuccess:ids=>setData(prev=>prev.filter(row=>!ids.includes(row.id))),onSave:()=>{console.log("안전보이스 저장",data)}})

const handleOpenQR=()=>{setQrUrl(`${window.location.origin}/public/safevoice`);setQrDialogOpen(true)}
const handleStateToggle=(id:number|string,newValue:string)=>setData(prev=>prev.map(r=>r.id===id?{...r,status:newValue,reason:newValue==="조치"?"조치 완료":r.reason}:r))
const handleInputChange=(id:number|string,key:string,value:string)=>setData(prev=>prev.map(r=>r.id===id?{...r,[key]:value}:r))
const handleSaveRow=(newItem:Partial<DataRow>)=>{setData(prev=>[{id:prev.length+1,status:"미조치",reason:"",sitePhotos:[],...newItem},...prev]);setModalOpen(false)}
...
<div className="overflow-x-auto bg-white"><DataTable columns={safeVoiceColumns}data={currentData}onCheckedChange={setCheckedIds}onStateToggleChange={handleStateToggle}onInputChange={handleInputChange}/></div>
