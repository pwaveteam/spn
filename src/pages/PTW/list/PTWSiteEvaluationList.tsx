...
import { SiteEvaluationItem } from "@/types/ptw"
import { siteEvaluationMockData } from "@/data/mockData"
...
const columns: Column<SiteEvaluationItem>[] = [
...
export default function PTWSiteEvaluationList() {
  ...
  const [data, setData] = useState<SiteEvaluationItem[]>(siteEvaluationMockData)
  const [checkedIds, setCheckedIds] = useState<(number | string)[]>([])
  const { currentPage, totalPages, currentData, onPageChange } = usePagination<SiteEvaluationItem>(data, 30)

  const { handleCreate, handleDelete, handleGenerateQR } = useTableActions({
    data,
    checkedIds,
    onCreate: () => navigate("/ptw/register", { state: { activeTab: 2 } }),
  ...
  return (
    <section className="w-full bg-white">
      ...
      <div className="overflow-x-auto bg-white">
        <DataTable<SiteEvaluationItem>
          columns={columns}
          data={currentData}
          onCheckedChange={setCheckedIds}
          onManageClick={() => navigate("/ptw/manage", { state: { activeTab: 2 } })}
        />
      </div>
