// pages/PTW/FilePanel/FilePanel.tsx
  ...
  return (
    <>
      <div className="h-full flex flex-col p-6">
        ...
        <div className="mb-4">
          <h3 className="font-semibold text-lg text-gray-800">첨부 파일</h3>
        ...
        <label 
          className={`cursor-pointer mb-6 block ${uploading ? 'opacity-50 cursor-not-allowed' : ''}`}
        ...
        >
          <div 
            className={`border border-dashed rounded-xl p-12 text-center transition ${
              isDragging 
                ? 'border-blue-400 bg-blue-100/50' 
                : 'border-gray-300 bg-slate-50 hover:border-blue-300 hover:bg-blue-50/40'
            }`}
          >
            <Upload className="mx-auto mb-3 text-gray-400" size={48} strokeWidth={1.5} />
            <p className="text-base font-medium text-gray-700 mb-1">
              {uploading ? '업로드 중...' : isDragging ? '파일을 여기에 놓으세요' : '파일을 여기에 드롭하거나'}
            </p>
            {!isDragging && <p className="text-blue-600 font-medium">클릭하여 선택</p>}
            <p className="text-xs text-gray-500 mt-3">이미지, PDF 파일 (다중 선택 가능)</p>
          </div>
          <input 
            type="file" 
            className="hidden" 
        ...
        <div className="flex-1 overflow-auto">
          {files.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-12">
              <FileText className="text-gray-300 mb-3" size={64} strokeWidth={1} />
            ...
            <div className="space-y-2">
              {files.map(file => (
                <div 
                  key={file.id} 
                  className="p-3 bg-white border border-gray-200 rounded-lg hover:shadow-sm transition"
                >
            ...
            <div className="relative">
              ...
              {previewFile.type?.startsWith('image/') ? (
                ...
                <div className="bg-white rounded-lg shadow-2xl p-12">
                  ...
                  <p className="text-gray-600">미리보기를 지원하지 않는 파일입니다</p>
                </div>
              )}
              ...
              <button 
                ...
                className="absolute top-2 right-2 inline-flex items-center justify-center w-8 h-8 rounded-md bg-black/60 text-white hover:bg-black/70"
              >
                <X size={16} />
              </button>
