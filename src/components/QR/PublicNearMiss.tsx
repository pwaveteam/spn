...
import { AlertTriangle, Camera, Image, X, CheckCircle } from "lucide-react"
...
export default function PublicNearMiss() {
  ...
  const handleSubmit = async () => {
    if (!form.place || !form.danger || !form.registrant) { alert("장소, 유해위험요인, 등록인을 입력해주세요."); return }
  ...
  const handleReset = () => { setForm({ place: "", danger: "", registrant: "", date: new Date().toISOString().split("T")[0] }); setPhotos([]); setIsSuccess(false) }
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex flex-col items-center justify-center p-6">
        <div className="bg-white rounded-xl p-8 text-center shadow-lg max-w-sm w-full">
          ...
          <p className="text-sm text-gray-500 mb-6">아차사고가 성공적으로 등록되었습니다.</p>
  ...
  return (
    <div className="min-h-screen bg-gray-50 pb-24">
      <header className="bg-[var(--primary)] text-white p-4 sticky top-0 z-10">
        <div className="flex items-center gap-2"><AlertTriangle size={24} /><h1 className="text-base font-semibold">아차사고 등록</h1></div>
      </header>
      ...
      <main className="p-4">
        <div className="bg-white rounded-lg border border-gray-200 p-4 mb-4">
          ...
          <div className="space-y-3">
            ...
            <div><label className="text-[10px] text-gray-500 mb-1 block">장소 *</label><Input placeholder="발생 장소 입력" value={form.place} onChange={e => handleChange("place", e.target.value)} className="text-xs h-9" /></div>
            <div><label className="text-[10px] text-gray-500 mb-1 block">유해위험요인 *</label><textarea placeholder="유해위험요인을 상세히 입력해주세요" value={form.danger} onChange={e => handleChange("danger", e.target.value)} className="w-full px-3 py-2 text-xs border border-gray-200 rounded-lg focus:outline-none focus:border-[var(--primary)] min-h-[80px] resize-none" /></div>
            <div><label className="text-[10px] text-gray-500 mb-1 block">등록인 *</label><Input placeholder="이름 입력" value={form.registrant} onChange={e => handleChange("registrant", e.target.value)} className="text-xs h-9" /></div>
          </div>
        </div>

        <div className="bg-white rounded-lg border border-gray-200 p-4">
          <h2 className="text-xs font-semibold text-gray-700 mb-3 flex items-center gap-2"><span className="w-1 h-4 bg-[var(--primary)] rounded-full" />현장 사진</h2>
          ...
          {photos.length === 0 && <p className="text-xs text-gray-400">사진을 추가해주세요</p>}
        </div>
      </main>
