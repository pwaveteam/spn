...
{ id: 1, category: "안전시설", item: "소화기 비치 상태", checked: null, note: "" },
{ id: 2, category: "안전시설", item: "비상구 표시등 작동", checked: null, note: "" },
{ id: 3, category: "안전시설", item: "안전난간 설치 상태", checked: null, note: "" },
{ id: 4, category: "전기설비", item: "분전반 관리 상태", checked: null, note: "" },
{ id: 5, category: "전기설비", item: "접지 상태", checked: null, note: "" },
{ id: 6, category: "작업환경", item: "정리정돈 상태", checked: null, note: "" },
{ id: 7, category: "작업환경", item: "통로 확보 상태", checked: null, note: "" },
]
...
const handleCheck = (itemId: number, value: boolean) => {
setCheckItems(prev =>
prev.map(item =>
...
setCheckItems(prev =>
prev.map(item =>
item.id === itemId ? { ...item, photo: ev.target?.result as string } : item
)
)
}
...
const unchecked = checkItems.filter(item => item.checked === null)
if (unchecked.length > 0) {
alert(`미점검 항목이 ${unchecked.length}개 있습니다.`)
return
}
...
const completedCount = checkItems.filter(item => item.checked !== null).length
...
<p className="text-gray-500 text-sm">점검 정보를 불러오는 중...</p>
</div>
)
}
...
>
<p className="text-sm font-medium text-gray-800 mb-3">{item.item}</p>
...
className="w-full px-3 py-2 text-xs border border-gray-200 rounded-lg mb-2 focus:outline-none focus:border-[var(--primary)]"
/>

<div className="flex items-center gap-2">
<button
onClick={() => handlePhotoCapture(item.id)}
className="flex items-center gap-1 px-3 py-1.5 text-xs text-gray-600 bg-gray-100 rounded-lg hover:bg-gray-200"
>
<Camera size={14} />
사진
</button>
...
className="w-full py-3 bg-[var(--primary)] text-white text-sm font-semibold rounded-lg hover:opacity-90 disabled:opacity-50 transition-opacity"
>
