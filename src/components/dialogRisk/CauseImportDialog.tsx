...
<div className="bg-white rounded-none md:rounded-2xl w-full md:w-[960px] md:max-w-full p-4 md:p-6 shadow-2xl h-screen md:h-[85vh] flex flex-col">
<div className="flex items-center justify-between mb-4 shrink-0">
<h2 className={`text-base md:text-xl font-bold tracking-tight ${TEXT_PRIMARY}`}>기인물요인 불러오기</h2>
...
<th className={`border-b ${BORDER_CLASS} border-l ${BORDER_CLASS} ${TH_PADDING} ${TEXT_SIZE_TH} font-medium ${TEXT_SECONDARY} w-28 md:w-40 text-center`}>위험분류</th>
<th className={`border-b ${BORDER_CLASS} border-l ${BORDER_CLASS} ${TH_PADDING} ${TEXT_SIZE_TH} font-medium ${TEXT_SECONDARY} text-left`}>위험발생 상황 및 결과</th>
<th className={`border-b ${BORDER_CLASS} border-l ${BORDER_CLASS} ${TH_PADDING} ${TEXT_SIZE_TH} font-medium ${TEXT_SECONDARY} w-40 md:w-60 text-left hidden md:table-cell`}>유해위험요인</th>
</tr>
</thead>
...
<td className={`border-b ${BORDER_CLASS} border-l ${BORDER_CLASS} ${CELL_PADDING} ${TEXT_SIZE_TD} ${TEXT_PRIMARY} text-center`}>{row.category}</td>
<td className={`border-b ${BORDER_CLASS} border-l ${BORDER_CLASS} ${CELL_PADDING} ${TEXT_SIZE_TD} ${TEXT_PRIMARY}`}>{row.situation}</td>
<td className={`border-b ${BORDER_CLASS} border-l ${BORDER_CLASS} ${CELL_PADDING} ${TEXT_SIZE_TD} ${TEXT_PRIMARY} hidden md:table-cell`}>{row.hazard}</td>
</tr>
))
) : (
<tr>
<td colSpan={4} className={`p-12 ${TEXT_SIZE_TD} text-gray-400 text-center`}>등록된 기인물요인이 없습니다.</td>
</tr>
)}
</tbody>
</table>
