...
export default function FormScreen({
  ...
  const FONT_XS_BASE = "text-xs md:text-base"
  ...
  const FILE_WRAPPER_CLASS = `w-full h-[38px] ${BORDER_CLASS} flex items-center justify-center ${TEXT_BASE_CLASS}`
  const FILE_BTN_CLASS = `h-[30px] flex items-center px-3 bg-gray-100 rounded ${FONT_XS_BASE} ${TEXT_PRIMARY} cursor-pointer`
  const FILE_TEXT_CLASS = `${FONT_XS_BASE} ${TEXT_SECONDARY} flex-1 truncate`
  ...
  const TAG_ITEM_CLASS = `flex items-center ${TEXT_PRIMARY} text-[11px] md:text-sm ${BORDER_CLASS} px-[9px] py-[3px]`
  const TAG_PLACEHOLDER_CLASS = `${FONT_XS_BASE} font-normal ${TEXT_SECONDARY} select-none ml-2`
  ...
  const renderInput = (field: Field) => {
        ...
        <div className="py-0">
          ...
          >
            {values[field.name] ? (
              ...
              <span className={`${TEXT_SECONDARY} ${FONT_XS_BASE}`}>서명 이미지 없음</span>
            )}
          </div>
        </div>
      )
    ...
    const renderDateInput = (name: string, value: string, required = false, disabled = false) => (
      <input
        ...
        value={value}
        onChange={onChange}
        className={`${disabled ? INPUT_READONLY : INPUT_EDITABLE} flex-1 min-w-[120px]`}
        style={BORDER_STYLE}
        disabled={disabled}
    ...
    const renderTimeSelect = (name: string, value: string, type: "hour" | "minute", required = false) => (
      <select
        ...
        value={value}
        onChange={onChange}
        className={`${INPUT_EDITABLE} ${SELECT_PADDING} flex-1 min-w-[70px]`}
        style={BORDER_STYLE}
        {...(required ? { required: true } : {})}
      >
      ...
      return (
        <div className="flex flex-col gap-2 w-full">
          ...
          <div className="flex items-center gap-1 w-full">
            ...
            <span className={`${FONT_XS_BASE} ${TEXT_PRIMARY} px-1`}>~</span>
    ...
    if (field.type === "timeRange") {
      return (
        <div className="flex flex-col gap-2 w-full">
          ...
          <span className={`${FONT_XS_BASE} ${TEXT_PRIMARY}`}>~</span>
    ...
    if (field.type === "daterange") {
      return (
        <div className="flex items-center gap-2 w-full">
          ...
          <span className={`${FONT_XS_BASE} ${TEXT_PRIMARY} shrink-0`}>~</span>
          ...
          {isCustomEmail ? (
            <>
              ...
              <button
                ...
                className="text-xs md:text-sm text-gray-500 underline shrink-0"
              >
                선택
              </button>
            </>
            ...
            <select
              ...
              className={`${INPUT_EDITABLE} ${SELECT_PADDING} flex-1 min-w-[120px]`}
              style={BORDER_STYLE}
            >
      ...
      return (
        <div className="flex items-center gap-2 w-full md:max-w-[300px]">
          <input
            ...
            className={`${INPUT_EDITABLE} flex-1`}
            style={BORDER_STYLE}
            onChange={onChange}
          />
          <select
  ...
  return (
    <section className={`w-full max-w-full bg-white ${FONT_XS_BASE}`}>
          ...
          <tbody>
            {fields.map(f => (
              <tr key={f.name} style={{ borderBottom: "1px solid var(--border)" }}>
                <th
                  className={`w-[100px] md:w-[160px] py-[8px] md:py-[11px] px-[8px] md:px-[13px] font-semibold text-left ${FONT_XS_BASE} ${f.type === "textarea" ? "align-top pt-3 md:pt-4" : "align-middle"}`}
                  style={{
                    ...headerFont,
                ...
                <td
                  className={`px-0 bg-white ${FONT_XS_BASE} align-middle`}
                  style={{
                    ...bodyFont,
